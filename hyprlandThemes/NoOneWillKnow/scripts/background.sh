...
if [ -n "$selected" ]; then
  ...
  sed -i '3s|.*|    path = '"${folder}${selected}"'|' ~/.config/hyprlandThemes/NoOneWillKnow/hypr/hyprpaper.conf
  sed -i '9s|.*|    path = '"${folder}${selected}"'|' ~/.config/hyprlandThemes/NoOneWillKnow/hypr/hyprpaper.conf
  killall hyprpaper
  hyprpaper -c ~/.config/hyprlandThemes/NoOneWillKnow/hypr/hyprpaper.conf  & sleep 1 && kitty +kitten panel --config=~/.config/kitty/kittyl.conf --edge=background -o background_opacity=0 -o background=black sh -c "printf \"\n\n\n\"; hyfetch; printf \"\e[?25l\"; sleep infinity" >/dev/null 2>&1 < /dev/null &
fi
